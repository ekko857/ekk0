...
# Extracting IncNodePurity importance data for each model
importance_N0_Purity <- data.frame(Variable = rownames(importance(RandomForest_model_N0)), IncNodePurity = importance(RandomForest_model_N0)[, "IncNodePurity"], Hill_Number = "N0")
importance_N1_Purity <- data.frame(Variable = rownames(importance(RandomForest_model_N1)), IncNodePurity = importance(RandomForest_model_N1)[, "IncNodePurity"], Hill_Number = "N1")
importance_N2_Purity <- data.frame(Variable = rownames(importance(RandomForest_model_N2)), IncNodePurity = importance(RandomForest_model_N2)[, "IncNodePurity"], Hill_Number = "N2")

# Combining the data frames
importance_combined_Purity <- rbind(importance_N0_Purity, importance_N1_Purity, importance_N2_Purity)

# Preparing data for plotting
importance_combined_Purity <- importance_combined_Purity %>%
  mutate(Variable = factor(Variable, levels = unique(Variable))) %>%
  group_by(Variable) %>%
  mutate(Average_IncNodePurity = mean(IncNodePurity)) %>%
  ungroup() %>%
  mutate(Variable = reorder(Variable, Average_IncNodePurity))

# Plotting the IncNodePurity importance across Hill Numbers
ggplot(importance_combined_Purity, aes(x = Variable, y = IncNodePurity, fill = Hill_Number)) +
  geom_bar(stat = "identity", position = position_dodge(), width = 0.7) +
  coord_flip() +
  labs(title = "Variable Importance by IncNodePurity across Hill Numbers",
       x = "Variable",
       y = "Increase in Node Purity") +
  scale_fill_brewer(palette = "Set3") +  # Using a different palette
  theme_minimal() +
  theme(legend.title = element_text(face = "bold"), legend.position = "top")
...
# Create data frame with environmental variables and their descriptions
table_11 <- data.frame(
  Variable = c("DHW (Degree Heating Weeks)", 
  ...
  Description = c("Measures accumulated heat stress over 12 weeks. Used to monitor coral bleaching events.",
                  "The temperature of the surface layer of the ocean. SST is a key indicator of ocean-atmosphere interaction.",
                  "The difference between the observed SST and the long-term average. Positive values indicate above-average temperatures.",
                  "Variation in SST over time. High variability may indicate seasonal changes or weather events.",
                  "The extent of sky covered by clouds, affecting solar radiation reaching the ocean surface.",
                  "Large-scale horizontal movement of water, important for the distribution of nutrients and heat.",
                  "The vertical distance from the surface to the bottom of the water body. Influences light penetration, temperature, and habitat distribution.",
                  "The concentration of dissolved salts in seawater. Affects water density and the physiology of marine organisms.",
                  "The speed and direction of horizontal air movement, influencing heat exchange and ocean surface conditions.")
)

# Create a table plot
table_plot <- tableGrob(table_11, rows = NULL)

# Save the table as an image
ggsave("environmental_variables_table.png", plot = table_plot, width = 14, height = 4, dpi = 300)
